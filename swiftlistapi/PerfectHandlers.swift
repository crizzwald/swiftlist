...
//    Routing.Routes["/foo/*/baz"] = { _ in return EchoHandler(listManagerProtocol: Managers.List()) }
//    Routing.Routes["/foo/bar/baz"] = { _ in return EchoHandler(listManagerProtocol: Managers.List()) }
//    Routing.Routes["GET", "/user/{id}/baz/{bazid}"] = { _ in return Echo2Handler() }
...
//        
//        response.appendBodyString("Echo handler: You accessed path \(request.requestURI()) with variables \(request.urlVariables): List: \(list.title)")
//        response.requestCompletedCallback()
//    }
//}
//
